...
for idx_n = 1:l_n
    for idx_q = 1:l_q
        ...
        % standard EL 
        q=q0;
        [~,fval_min_std] = fmincon(fun,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n),options);
        [~,fval_max_std] = fmincon(fun2,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n),options);
        % estimated Bartlett
        q = q0*(1+A/n);
        [~,fval_min_BL] = fmincon(fun,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n),options);
        [~,fval_max_BL] = fmincon(fun2,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n),options);
        % theoretical Bartlett
        q = q0*(1+At/n);
        [~,fval_min_TBL] = fmincon(fun,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n),options);
        [~,fval_max_TBL] = fmincon(fun2,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n),options);
