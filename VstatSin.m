...
CIlength = zeros(l_n,l_q,N,3);%saves the length of the confidence intervals
h = @(x,y) (sin(x^2+y)+sin(x+y^2))/2; % the objective function for V statistics
...
% estimate the coverage probability and CI width of different methods
for idx_n = 1:l_n
    for idx_q = 1:l_q
        n = ns(idx_n);%number of samples per run
        q0 = qs(idx_q); 
        
        % compute the "true" adjustment factor by estimating using a large sample
        % size
        nt=5000;
        r = trnd(dof,nt,1);
        ...
        parfor rep = 1:N
            ...
            % standard EL 
            q=q0;
            [~,fval_min_std] = fmincon(fun,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n));
            [~,fval_max_std] = fmincon(fun2,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n));
            % estimated Bartlett
            q = q0*(1+A/n);
            [~,fval_min_BL] = fmincon(fun,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n));
            [~,fval_max_BL] = fmincon(fun2,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n));
            % theoretical Bartlett
            q = q0*(1+At/n);
            [~,fval_min_TBL] = fmincon(fun,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n));
            [~,fval_max_TBL] = fmincon(fun2,l0,[],[],ones(1,n),n,zeros(n,1),[],@(l)chidiv(l,q,n));
